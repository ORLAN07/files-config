...
A SAM Lambda service (Python 3.8) that swaps case of text. Text becomes tEXT.
...
aws cloudformation deploy --template-file serverless-output.yaml --stack-name swapcaser --capabilities CAPABILITY_NAMED_IAM
...
sam deploy --template-file serverless-output.yaml --stack-name swapcaser --capabilities CAPABILITY_IAM
4. Once Cloudformation is deployed, use aws cloudformation describe-stacks --stack-name swapcaser, or the AWS Cloudformation console, to retrieve the endpoint address.
